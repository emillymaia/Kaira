struct TestPhaseStructure {
    ...
    init(_ customDelegate: DataDelegate) {
        let testGamePhaseModel = GamePhaseModel(countryName: "England", background: "pause-background", assets: [
        ...
        let testGame = DragAndDropViewController()
        testGame.gamePhaseModel = testGamePhaseModel
        testGame.customDelegate = customDelegate
        ...
        self.historyPages = [
            HistoryPageModel(
                ...
                text: texts[0],
                button: .next,
                skipViewController: testGame
            ),
            HistoryPageModel(
                image: "england-2",
                text: texts[1],
                button: .finish,
                nextViewController: testGame,
                skipViewController: testGame
            ),
            HistoryPageModel(
                image: "england-3",
        ...
        testGame.historyViewController = self.historyVC[1]
    }
}
